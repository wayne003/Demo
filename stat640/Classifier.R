...
for (k in 1:70){
    ...
    ## Xts=subset(MyData2,MyData2$Sentence==k)
    ...
    for (i in 1:140){
        ...
        Mcor =max(ccf(Xtra,Xtsa,plot = FALSE,type="correlation")$acf)+0.1
        Result[i,2]=ifelse(Mcor>0,Mcor,0)
    }
    ...
    ## Result1=subset(Result,Result$Sentence!=k)
                                        ...
                                        #Result1=Result     
    Result1=Result[order(Result[,1],decreasing = FALSE),]
...
saveRDS(CorResult,file='/scratch/zz38/CorResultTest.rds')
## saveRDS(CorResult,file='/scratch/zz38/CorResultTrain.rds')
